...
func TestHunkExit(t *testing.T) {
	for _, c := range []struct {
		...
		// --try exits with its command's status, which is not hunk's verdict.
		{"--try, and the command passed", "hunk --try 'zig build' -f p", "tried 1 hunk and put back 1 file; the command exited 0 (0.4s)\n", 0},
		{"--try, and the command exited 3", "hunk --try 'zig build' -f p", "hunk: tried 1 hunk and put back 1 file; the command exited 3 (0.4s)\n", 0},
		{"--try, and a file was left alone", "hunk --try x -f p", "hunk: tried 2 hunks and put back 1 file, 1 file left alone; the command exited 0 (0.1s)\n", 4},
		{"--try, and the command could not start", "hunk --try x -f p", "hunk: could not run the --try command: exec: \"sh\": not found; the batch was put back\n", 5},
		{"a verify whose command could not start", "hunk --verify x -f p", "hunk: could not run the verify command: fork/exec sh: exec format error; the batch was rolled back\n", 5},
		{"no sh, found before writing", "hunk --verify x -f p", "hunk: --verify runs its command with sh, which was not found (exec: \"sh\": executable file not found in $PATH); nothing was written\n", 5},
		{"no sh, under --try", "hunk --try x -f p", "hunk: --try runs its command with sh, which was not found (x); nothing was written\n", 5},
		{"--try --json whose command exited 3", "hunk --json --try x -f p", "{\n  \"ok\": false,\n  \"exit\": 3,\n  \"try\": {\n    \"ran\": true,\n    \"status\": 3\n  }\n}\n", 0},
		{"--try --json with a file left alone", "hunk --json --try x -f p", "{\n  \"exit\": 4,\n  \"try\": {\n    \"ran\": true,\n    \"not_restored\": []\n  }\n}\n", 4},
		{"--try --json whose command could not start", "hunk --json --try x -f p", "{\n  \"exit\": 5,\n  \"error\": \"could not run the --try command\",\n  \"try\": {\n    \"ran\": false\n  }\n}\n", 5},
		// A command that exits 5 with the tree back is still hunk's 0.
		{"--try --json whose command exited 5", "hunk --json --try x -f p", "{\n  \"exit\": 5,\n  \"try\": {\n    \"ran\": true,\n    \"status\": 5\n  }\n}\n", 0},
...
var reportGoldens = map[string]int{
	...
	"report-try-passed":               0,
	"report-try-passed-json":          0,
	"report-try-failed":               0,
	"report-try-failed-json":          0,
	"report-try-left-alone":           4,
	"report-try-left-alone-json":      4,
